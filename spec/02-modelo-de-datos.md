...
| status | text NOT NULL default 'CONNECTED' | `CONNECTED` \| `DISCONNECTED` (pausada, token conservado) \| `ERROR` (token muerto) \| `UNSUPPORTED` \| `REMOVED` (dada de baja, credencial borrada) |
...
> **Ciclo de vida (ver [[09-flujo-oauth]]).** `DISCONNECTED` = pausa **deliberada**, la credencial **se
> conserva** (reconectar puede reactivar con un refresh, sin OAuth). `ERROR` = token muerto, requiere
> re-autorización. `REMOVED` = **baja**: la credencial se **borra** y el token se **revoca** en la red
> (best-effort), pero la **fila y la historia se conservan** (soft-delete) para reportes pasados; la UI
> de "cuentas conectadas" no lista las `REMOVED`. El job solo sincroniza `CONNECTED`. Re-agregar una
> cuenta `REMOVED` reusa la fila vía `UNIQUE(platform, external_account_id)`.

### account_credentials
Token OAuth de larga duración. 1:1 con `social_accounts`. Separada para rotación/auditoría.
Se **conserva** en `DISCONNECTED`/`ERROR` y se **borra** al dar de baja la cuenta (`REMOVED`); el
borrado de la cuenta cascada a su credencial. Ver ciclo de vida en [[09-flujo-oauth]].
| Columna | Tipo | Notas |
|---|---|---|
...
### connect_links
Link temporal para **onboarding self-service**: el cliente conecta su red desde su propio navegador
sin login en Fil-Grama (ver [[09-flujo-oauth]]). **No guarda tokens**; solo habilita iniciar el OAuth
acotado a un cliente.
| Columna | Tipo | Notas |
|---|---|---|
| id | bigint PK | |
| client_id | bigint FK → clients(id) NOT NULL | tenant al que se ligará la cuenta conectada |
| token_hash | text UNIQUE NOT NULL | hash (sha-256) del token opaco; el raw va en la URL y se devuelve **una sola vez** |
| platform | text NULL | fija la red (`INSTAGRAM`\|`FACEBOOK`\|`TIKTOK`); NULL = el cliente elige |
| expected_account_id | bigint FK → social_accounts(id) NULL | reconexión de una cuenta puntual (hereda el guard del `open_id` esperado) |
| created_by | bigint FK → users(id) NOT NULL | quién generó el link; pasa a `social_accounts.connected_by` |
| expires_at | timestamptz NOT NULL | TTL (default `now() + 72h`) |
| revoked_at | timestamptz NULL | desactivación manual |
| used_at | timestamptz NULL | último uso exitoso (auditoría; el link es **multi-uso** hasta expirar/revocar) |
| created_at | timestamptz NOT NULL default now() | |
| | | UNIQUE (token_hash) ; INDEX (client_id) |

> **Por qué `token_hash` y no el token.** Igual que un token de reset de contraseña: si se filtra la
> DB, los links activos no quedan expuestos. El backend hashea el token entrante y busca por `token_hash`.
> Token ≥256 bits URL-safe, no adivinable. TTL corto + revocación acotan la ventana. El token raw nunca
> se persiste ni se loguea; se devuelve solo en la respuesta de creación.
